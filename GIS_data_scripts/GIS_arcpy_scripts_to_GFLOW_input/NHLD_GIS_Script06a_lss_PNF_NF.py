...
for curID in IDs:
    ...
    for row in cursor:
        ...
        LSS_Area = row.getValue('Area_m2_S')
        LSS_Depth = (10**(-0.0589+1.12963*math.log10(LSS_Area)))/LSS_Area
        LSS_Width = row.getValue('Width_m_S')
...
##    #import numpy as np #(already did in call file)
##    X_pts = np.array(LSS_Vert_X)
##    Y_pts = np.array(LSS_Vert_Y)
##    vert_dist = np.zeros((len(X_pts),len(X_pts)))
##
...
##                vert_dist[i,j] = 0
##            else:
##                vert_dist[i,j] = np.sqrt(np.power((X_pts[j]-X_pts[i]),2) + np.power((Y_pts[j]-Y_pts[i]),2))
##    #Takes mean between all vertices (it is know that how this is currently written has redudant information bc distance between two points is the same, but the means will be the same either way;
##    #This should be better underbound estimate than using LSS_Width = row.getValue('MBG_Width')
##    LSS_Width = np.mean(vert_dist[np.nonzero(vert_dist)])
...
print('Complete PNF_NF lss xml create')
