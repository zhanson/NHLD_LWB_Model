%NHLD_LWB_Model.m
%Previous internal Version: LakeModel_MAIN_Rev23_GEN_Cleaned.m

%% Code Sections Outline:
%0)     Set Paths and variables for current HUC12
%0a)    Set Input Data Paths
%0b)    Set LWB Simulation Duration and GFLOW Aquifer Parameters

%1)     Read VIC and GFLOW initial files for LWB setup for current HUC12
%1a)    VIC Flux Output Format
%1b)    Import Current Huc12 nearfiled GIS information, Calc AQ_Base_Elev
%1c)    Import Current Huc12 primary-nearfiled GIS information for post-processing regional synthesis
%1d)    Read GIS_Label_to_GFLOW_LS_Lookup
%1e)    VIC output to GFLOW recharge input for all monthly (time) intervals     

%2)     Setup Monthly GFLOW, Daily LWB Runs 
%2a)    Read x1,y1,x2,y2, and label vectors from original Initial .dat file
%2b)    Read width vector from original Initial .dat file
%2c)    Read Other data from original Initial .dat file
%2d)    Read lakebed resistance and depth vectors from GFLOW Initial .xtr file

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %Begin Main (Monthly) Loop for current HUC12
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
%3)     Write GFLOW input, run        
%3a)    Set/Modify lake water-surface elevations for GFLOW monthly run     
%3b)    Set/Modify lakebed Resistance for GFLOW monthly run      
%3c)    Write NEW GFLOW monthly RUN .dat file
%3d)    Write .bat file to run monthly GFLOW RUN
%3e)    Exceute GFLOW monthly RUN .bat file     

%4)     Read GFLOW output, calc daily GW discharges for LWB input 
%4a)    Open new monthly GFLOW .xtr file
%4b)    Extract TP Data ("Test-Point" or Wells; COMMENTED OUT, can use if TP placed in HUC12 domain, such as TR)
%4c)    Extract LS Data
%4d)    Calculate LineSink String Discharges (GW in and out for each lake current HUC12)

%5)     Run LWB model 
%5a)    Initialize properties and forcings to run LWB model for all nearfield lakes within current HUC12 sub-domain
%5a)    Run Lake Model; k==1 (first WY, elevations at DEM), Initialize Lake Variables
%5c)    Run Lake Model; k>1 (1< WY)

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %End Main (Monthly) Loop for current HUC12
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
%6)     Summarize (runtime,drying lakes (pre-screening relevant ONLY))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Uncomment to run only TR HUC12:
% % close all
% % clear
% % clc
% % Cur_HUC12 = '070500020105'; %Trout Lake Watershed HUC12 ID
...
Inpath_Input = './Input_Datasets/'; %pre-screening datasets
Outpath_GFLOW_tmp_run = './GFLOW_tmp_run_files/'; %location for tmp GFLOW run files

%VIC Flux and Lake files:
...
%pre-screening .mat files:
load(fullfile(Inpath_Input,'NHLD_LakeClassification_withStreamSheds.mat'));
load(fullfile(Inpath_Input,'NHLD_GW_Disconnect_IDs.mat')); %GIS PERMs of NHLD PNF_NF lakes that failed loads variable "NHLD_GW_Disconnect_IDs
load(fullfile(Inpath_Input,'In_IDs_SW_Fill_Rev16_Apr2005_maxSWinBase_V_r1.mat')); %GIS PERMS of NHLD PNF_NF lakes that Apr 2005 max(SWin + Base)/ V0  >= 1 (meaning lake completely filled in 1 event), loads variable "In_IDs_SW_Fill"
load(fullfile(Inpath_Input,'WALA_adjusted_lookup_Rev2.mat'));
...
%Running entire NHLD in serial for WYs 1980-2015 will take between 4-6 days on a single standard PC.
%Alternatively, you can run several of the "Loop" scripts to run this .m file and 
%break up the domain to run in parallel on separate PCs

time_window_size = 1; %default = 1 (monthly); size of simualtion interval in months (1+, factors of 12)
...
StdGW = 1; %1 == ON (default), 0 == OFF; If ON, using standard constant monthly GW. If Off, inter monthly variability (testing ONLY)
Lake_Dry_Threshold = 0; %0 (default)
...
%% 1d) Read GIS_Label_to_GFLOW_LS_Lookup:
%Must run GFLOW once, copy/paste Linesink
...
NumDaysTot = daysdif(Date_b,Date_e,0) + 3;
...
%we need in this script. I.e. Using the initial .dat file as a template, we
%will write unique .dat files for each monthly GFLOW iteration with updated
%recharge and lake water-surface elevations
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% MAIN LOOP (time (Monthly) period loop)
eltime = [];
Lake_fail_dry = [];
...
for k = 1:length(GFLOW_NetRechargeIN); %Number of GFLOW simulations
    ...
    %If k ==1 then use the DEM values to initialize
    %Else, update with the final elevation calculated from the previous
    %monlthy LWB model run.
    
    if k == 1
        z_new = LS_Ini_dat_z; %(DEM values)
    ...
    %% 3b) Set/Modify lakebed Resistance for GFLOW monthly run 
    %Parts of this were for pre-screening ONLY,
    %Final runs result in no additional drying lakes
    ...
    cd(Outpath_GFLOW_tmp_run); %location for tmp GFLOW run files
    ...
    %% 4a)   Open new monthly GFLOW .xtr file
            %(used in next steps to extract TestPoint and LineSink data
            %from GFLOW RUN# .xtr data)
...
% %     GFLOWout_results.(t_interval_char).TP_Label = TP_xtr_out_label;
    ...
    %change out of GFLOW run dir
    cd ..
    
    
    %% 4d) Calculate LineSink String Discharges
    if StdGW == 1 %else, inter monthly variability (which did not work well, use StdGW == 1)
    ...
    %% 5b)  Run Lake Model; k==1 (first WY, elevations at DEM), Initialize Lake Variables
    ...
    if k == 1 %If first timeRUN (WY)
        ...
        A = zeros(length(GIS_PERMANENT_),NumDaysSub);
