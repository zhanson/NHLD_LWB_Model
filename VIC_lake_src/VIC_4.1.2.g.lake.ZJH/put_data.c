...
**********************************************************************/
{
  ...
  ****************************************/
  ...
  if( rec == 0 ){
    out_data[OUT_GRND_RECHARGE].data[0] = 0.;
    save_data->ground_recharge = out_data[OUT_SOIL_MOIST].data[options.Nlayer-1];
  } else if (rec > 0)  {
    out_data[OUT_GRND_RECHARGE].data[0] = (out_data[OUT_SOIL_MOIST].data[options.Nlayer-1] - save_data->ground_recharge) + out_data[OUT_BASEFLOW].data[0] + out_data[OUT_GRND_EVAP].data[0] ;     
    save_data->ground_recharge = out_data[OUT_SOIL_MOIST].data[options.Nlayer-1];
  }

  //printf(" Recharge :  %d  %.2f  %.2f %.2f %.2f\n", rec, save_data->ground_recharge , out_data[OUT_SOIL_MOIST].data[options.Nlayer-1], out_data[OUT_GRND_RECHARGE].data[0], out_data[OUT_GRND_EVAP].data[0] ); 
  ...
  if (step_count == out_step_ratio) {
    ...
    ***********************************************/
    if (options.ALMA_OUTPUT) {
      ...
      out_data[OUT_GRND_RECHARGE].aggdata[0] /= out_dt_sec;
      out_data[OUT_GRND_EVAP].aggdata[0] /= out_dt_sec;
      out_data[OUT_TOP_EVAP].aggdata[0] /= out_dt_sec;
      out_data[OUT_VADOSE_EVAP].aggdata[0] /= out_dt_sec;
                      ...
                      out_data_struct  *out_data)
{
  ...
  out_data[OUT_GRND_EVAP].data[0] += cell.layer[options.Nlayer-1].evap * AreaFactor; 
  out_data[OUT_TOP_EVAP].data[0] += cell.layer[0].evap * AreaFactor; 
  out_data[OUT_VADOSE_EVAP].data[0] += cell.layer[1].evap * AreaFactor; 
